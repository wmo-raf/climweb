...
// Add zoom and rotation controls to the map.
        ...
        map.on("load", () => {
            if (homeMapSettingsUrl) {
                fetch(homeMapSettingsUrl).then(response => response.json()).then(settings => {
                    const {zoom_locations} = settings

                    if (zoom_locations && !!zoom_locations.length) {
                        map.addControl(new ZoomToLocationsControl(zoom_locations), 'top-right');
                    }

                    const defaultZoomLocation = zoom_locations.find(loc => loc.default)
